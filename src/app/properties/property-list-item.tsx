...
import { BathIcon, BedIcon, HomeIcon, SofaIcon, StarIcon } from 'lucide-react';
...
export function PropertyListItem({
  ...
  return (
    <Link className="w-full" href={`/properties/${id}/${getSlug(name)}`}>
      <div
        className={cn(
          'w-full flex flex-col md:flex-row bg-neutral-700 shadow-xl',
          featured && 'border-b-2 border-primary'
        )}
      >
        ...
        <div className="basis-3/4 p-4 md:p-8 flex flex-col gap-3 md:gap-6 justify-between">
          ...
          <div className="text-sm">{descText}</div>
          <div className="flex justify-between gap-4 pt-3">
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
              <Badge variant="secondary" className="uppercase">
                {getAvailabilityLabel(transactionType as TransactionType, status as PropertyStatus)}
              </Badge>
